...
def get_gan_loss(opt):
    ...
    elif opt.gan_loss == "wgan-eps":
        return ImprovedWGANEpsLoss
    else:
        raise ValueError
...
class ImprovedWGANEpsLoss(GANLoss):
    def __init__(self, discriminator, lambda_=10.0, eps_=1e-3):
        self.discriminator = discriminator
        self.lambda_ = lambda_
        self.eps_ = eps_
    ...
    def _discriminator_loss_logits(self, d_real, d_fake, x_real, x_fake, forward=None):
        grad_penalty = self.gradient_penalty(x_real, x_fake, forward=forward)
        return d_fake.mean() - d_real.mean() + self.lambda_ * grad_penalty + self.eps_ * d_real.mean() ** 2
