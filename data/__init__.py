...
def create_dataset(opt, phase='train', fold=None, load_vid=False, load_rgb=True, verbose=True):
    dataset = find_dataset_using_name(opt.dataset)
    instance = dataset(opt, phase=phase, fold=fold, load_vid=load_vid, load_rgb=load_rgb, verbose=verbose)
    file_type = "vid" if opt.from_vid else "img"
    load_type = "vid" if load_vid else "img"
...
def custom_collate_fn(batch):
    ...
    elem = batch[0]
    for key in elem:
        if key in ["img"] and elem[key].ndim == 4:
